...
- added explore() methods for baseline contrasts, complement contrasts,
  paired baseline contrasts, and correlations
- added dig_ancestors()
- added plot_contingency()
- enhanced info and error messages
- added arguments for line type and colour to geom_diamond()
- enhanced dig\_\*\_contrasts() - statistical test errors are now stored
...
- deprecated measures argument in dig_associations() and
  dig_tautologies()
...
- all dig\_\*() functions now return nugget S3 objects
- added is_almost_constant(), remove_almost_constant(),
  parse_condition(), fire(), is_condition(), remove_ill_conditions(),
  shorten_condition()
...
- added exclude argument to dig() and other dig\_\*() functions
...
- implemented: dichotomize(), dig(), dig_implications(),
  dig_correlations(), which_antichain(), format_condition(), is_subset()
