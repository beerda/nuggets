...
search with a custom callback. The callback computes the confidence of each
remaining focus and returns only those foci that pass the support and confidence
thresholds:
...
mtcars_disj <- var_names(colnames(fuzzy_mtcars))
...
rule_callback <- function(condition, pp, support) {
    ...
    lapply(seq_along(conf), function(i) {
        list(
            antecedent = format_condition(names(condition)),
...
- `weights` tell you how strongly each row satisfies the condition.
