...
- `indices`: row indices of the original dataset `x` satisfying the condition
  ...
  searches.
- `degrees`: per-row truth degrees of the condition in dataset `x`; logical
  (crisp) data is treated as 0/1 degrees.
...
- use `degrees` for custom fuzzy summaries,
...
to compute are `indices` and `degrees`. They require scanning the entire dataset
...
`indices` and `degrees` play different roles:

- `indices` tell you which rows have a non-zero truth degree for the condition,
- `degrees` tell you on scale $[0, 1]$ how strongly each row satisfies the condition.
...
fuzzy_callback <- function(condition, indices, degrees) {
    ...
    list(condition = format_condition(names(condition)),
         ...
         weighted_support = sum(degrees) / nrow(fuzzy_iris),
         mean_petal_length_by_indices = mean(iris$Petal.Length[indices]),
         mean_petal_length_by_degrees = weighted.mean(iris$Petal.Length, degrees))
}
...
equally. The weighted mean based on `degrees` respects the fuzzy truth degrees,
...
`degrees` quantify the strength of their membership.
...
- For fuzzy searches, use `degrees` when truth degrees matter, and `indices`
