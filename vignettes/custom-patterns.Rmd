...
numeric fuzzy predicates. We will use `iris` and prepare two small predicate data
sets that are rich enough for the examples below:
...
The commands above create crisp and fuzzy predicates for the four numeric columns
of `iris`, plus the three species predicates. The preparation step is
intentionally brief here; the dedicated `vignette("data-preparation")` explains
`partition()`, fuzzy predicates, and breakpoints in detail.
...
- `condition`:
  ...
  specifying which columns of `x` may appear in generated conditions.
- `focus`: 
  [tidyselect expression](https://tidyselect.r-lib.org/reference/language.html)
  specifying which columns of `x` are evaluated as foci within each condition.
