...
remove uninformative columns and discover tautological rules that can serve as
axioms for pruning subsequent pattern searches.
...
# Finding Tautologies and Using Axioms for Pruning
...
then serve as **axioms** for pruning redundant conditions in subsequent pattern
discovery via the `excluded` argument.


## What are Tautologies (Axioms)?
...
Once discovered, such rules can be treated as axioms: logical implications that 
are assumed to always hold. They are passed to the `excluded` argument of `dig()`, 
`dig_associations()`, and related functions to prune rules that are trivially 
deducible via modus ponens.


## Using `dig_tautologies()`

The `dig_tautologies()` function works similarly to `dig_associations()`, but 
is specifically designed for finding rules with very high confidence. It 
searches iteratively, using tautologies found in earlier iterations as axioms 
to prune the search space in later iterations.
...
## Using Tautologies as Axioms to Filter Searches

Once you've identified tautologies, you convert them to the axiom format 
expected by the `excluded` argument using `parse_condition(antecedent, consequent)`,
and pass the result to `dig()` or related functions.

The `excluded` argument accepts a list of character vectors, where each vector 
encodes one axiom: the last element is the consequent and all preceding elements 
are the antecedent. Rules are pruned via modus ponens:

- A rule's condition (antecedent) is pruned if it contains a predicate that can 
  be deduced from the other condition predicates using any axiom. This happens 
  when the condition contains all antecedent predicates of some axiom together 
  with the consequent—the consequent is then redundant.
  
- A rule's focus (consequent) is pruned if it can be deduced from the condition 
  predicates. This happens when the condition contains all antecedent predicates 
  of some axiom and the focus equals the consequent of that axiom.

```{r, eval=FALSE}
# Convert tautologies to axiom format for the excluded argument
axioms <- parse_condition(tautologies$antecedent, tautologies$consequent)
...
results <- dig_associations(
    ...
    disjoint = disj,
    excluded = axioms,  # Prune rules deducible from known axioms
    min_support = 0.1,
    min_confidence = 0.8
)
```
...
3. **`dig_tautologies()`**: A function for finding tautological rules (axioms) in
   your data. The results can be converted to the format expected by the
   `excluded` argument of `dig()` and related functions via
   `parse_condition(result$antecedent, result$consequent)`, enabling
   modus-ponens-based pruning of trivially deducible rules.
