...
```{r}
# Create a simple dataset with a logical column
x <- tibble::tibble(flag = c(TRUE, FALSE, NA, TRUE, FALSE))
partition(x, flag)
```
...
For example, with an unordered factor:

```{r}
# Create an unordered factor
x <- tibble::tibble(color = factor(c("red", "green", "blue", "red", "green")))

# Default: one predicate per level
partition(x, color)

# Merge pairs of levels - all pairs are created for unordered factors
partition(x, color, .subsets = 2)

# Create both individual and merged predicates
partition(x, color, .subsets = c(1, 2))
```

For ordered factors, only consecutive levels are merged:

```{r}
# Create an ordered factor
x <- tibble::tibble(size = ordered(c("small", "medium", "large", "medium"),
...
partition(x, size, .subsets = 2)

# Both individual levels and consecutive pairs
partition(x, size, .subsets = c(1, 2))
```
...
The `vs`, `am`, and `gear` columns in `mtcars` are numeric but actually represent
