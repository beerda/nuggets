...
columns. The `partition()` function handles all types of input columns:

- **logical** columns are automatically expanded into predicates for `TRUE` and
  `FALSE`;
- **factor** columns are expanded into predicates for subsets of their levels;
- **numeric** columns are transformed according to the `.method` argument.
...
### Logical and Factor Columns

Logical and factor columns are automatically transformed to dummy logical columns
...
By default, `partition()` creates one predicate per factor level (i.e.,
`.subsets = 1`). However, it is also possible to create predicates that
represent **merged factor levels** by setting the `.subsets` argument to
larger values. This is useful when you want to explore patterns involving
...
For example, with an unordered factor having levels `a`, `b`, `c`:
...
1. **`partition()`**: The primary function for transforming data into crisp or 
   ...
   - Logical and factor columns: automatic expansion into dummy predicates,
     with the ability to merge factor levels using the `.subsets` argument
   - Numeric columns: crisp (Boolean) partitioning with configurable intervals
