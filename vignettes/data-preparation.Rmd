...
- **`antecedent`** and **`consequent`**: Tidyselect expressions defining which 
  columns can appear on each side of the rule. These correspond to `condition`
  and `focus` in the general `dig()` function; the terms are interchangeable
  across the `dig_*()` family.
...
- **`max_length`**: The maximum number of predicates in the antecedent (condition).
...
condition (antecedent) and the last element is the focus (consequent):
