...
# Finding Tautologies and Using Them as Axioms
...
then be used as **axioms** to prune redundant conditions in subsequent pattern
discovery.
...
Such tautological rules can be treated as **axioms**: known implications that
the framework uses to prune uninteresting or redundant patterns via
**modus ponens**. Two types of pruning are applied (transitively, possibly via
a chain of multiple axiom applications):
...
Once you've identified tautologies, convert them to the axiom format expected
by the `excluded` argument using `parse_condition()`, and pass them to
`dig()` or related functions:
