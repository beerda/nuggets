...
- A rule's condition (antecedent) is pruned if it contains a predicate that can 
  be deduced from the other condition predicates using the axioms, possibly via 
  a chain of multiple axiom applications (transitive deduction). Such a predicate
  is redundant.
  
- A rule's focus (consequent) is pruned if it can be deduced from the condition 
  predicates using the axioms, possibly via a chain of multiple axiom applications
  (transitive deduction).
