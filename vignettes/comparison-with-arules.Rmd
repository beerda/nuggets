---
title: "Comparison of nuggets performance with arules nad fim4r"
...
two_fig <- function(d, x, xname, title, pkg) {
    ...
    d <- d |>
        filter(grepl(pkg, as.character(method), fixed = TRUE) | method == "nuggets") |>
        mutate(method = fct_relevel(method, "nuggets"))
...
- `fim4r` `r data$fim4r_version` (installed and executed via `arules`)
...
`nuggets`, `fim4r`, and `arules` packages. For `arules`, two different algorithms
were evaluated: the Apriori algorithm (`apriori()`) and and the Eclat algorithm 
(`eclat()`). For `fim4r`, five different algorithms were evaluated: Apriori, Eclat,
FP-Growth, Relim, and SAM (`fim4r` algorithms were executed via the `arules`
package, see `arules::fim4r()`). For `nuggets`, the `dig_associations()` function
was used to  discover association rules.
...
As can be seen from the results, `nuggets` performance is consistently among
the best. Only for sparse data with many columns, `arules:apriori()` becomes
clearly more efficient.

`nuggets` is based on the ECLAT algorithm similarly as `arules:eclat()` or
`fim4r::fim4r_eclat()`. Therefore, both variants are expected to perform similarly
well. A likely explanation for the strong performance of `nuggets` on dense data
