...
implications (axioms), where each element is a character vector of the form
`c(a1, a2, ..., an, c)` encoding the axiom `a1 & a2 & ... & an => c` (the
last element is the consequent). These axioms are used to prune rules via
**modus ponens** — transitively, possibly via a chain of multiple axiom
applications:

- **Condition pruning**: a condition is dropped if it contains all antecedent
  predicates of some axiom together with the consequent of that axiom, since
  the consequent is then redundant (it can be deduced from the remaining
  predicates).
- **Focus/consequent pruning**: a focus (or rule consequent) is dropped if it
  equals the consequent of some axiom whose antecedent predicates are all
  present in the condition (the focus is already implied by the condition).
...
Once tautologies are identified, convert them to the axiom format expected by
the `excluded` argument using `parse_condition()`, and pass them to
`dig_associations()`:

```{r}
# Convert tautologies to axiom format
...
character vector `c(a1, ..., an, c)` encoding the axiom `a1 & ... & an => c`:
