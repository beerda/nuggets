...
For the overall package workflow, see `vignette("nuggets")`. For creating
predicate columns, see `vignette("data-preparation")`.
...
```{r}
baseline_result <- dig_baseline_contrasts(
    iris_contrasts,
...
different from zero. Typical output columns are:
...
- `statistic`, `p_value`,
- `n`,
- confidence-interval bounds and test metadata.
...
```{r}
baseline_wilcox <- dig_baseline_contrasts(
    iris_contrasts,
...
condition identifies an unusual subgroup.

```{r}
complement_result <- dig_complement_contrasts(
    iris_contrasts,
    condition = where(is.logical),
...
- `estimate_x` for rows satisfying the condition,
- `estimate_y` for rows not satisfying the condition,
...
```{r}
complement_var <- dig_complement_contrasts(
    iris_contrasts,
    condition = starts_with("Species"),
    vars = Petal.Length,
    min_length = 1,
...
right", or two alternative measurements recorded for the same case.

```{r}
paired_result <- dig_paired_baseline_contrasts(
    iris_contrasts,
    condition = where(is.logical),
...
```{r}
paired_wilcox <- dig_paired_baseline_contrasts(
    iris_contrasts,
    condition = starts_with("Species"),
    xvars = Sepal.Length,
    yvars = Petal.Length,
...
As with other pattern-mining functions in `nuggets`, it is useful to keep track
of mutually exclusive predicates with `var_names()`:

```{r}
disj <- var_names(colnames(iris_contrasts))
disj[1:8]
```

For datasets prepared with `partition()`, this helps prevent contradictory
conditions built from predicates originating from the same variable.
...
complement_result[, c("condition", "x_name", "p_value", "p_holm", "p_bh")]
```
