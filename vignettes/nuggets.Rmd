...
## A More Complex Example: CO2 Dataset

Let's look at a more interesting example using the built-in `CO2` dataset, which 
contains data from an experiment on the cold tolerance of the grass species 
*Echinochloa crus-galli*. We'll prepare the data and discover some patterns.
...
We can prepare this data using different partitioning strategies for different 
columns. For the concentration (`conc`) variable, we'll use raised-cosine fuzzy 
sets to model smooth transitions. For the uptake rate, we'll use meaningful 
labels for the fuzzy sets:
...
prepared_CO2 <- CO2 |>
    ...
    partition(uptake,
              ...
              .labels = c("minimal", "low", "moderate", "high", "maximal"))

head(prepared_CO2, n = 3)
```

Now we can search for association rules. For instance, we might want to find 
which combinations of plant type, treatment, and concentration levels are 
associated with high uptake rates:

```{r}
# Create disjoint vector to avoid nonsensical conditions
disj <- var_names(colnames(prepared_CO2))

# Search for rules predicting high uptake
rules <- dig_associations(
    prepared_CO2,
...
# Show the most interesting rules
rules |>
    dplyr::arrange(desc(confidence), desc(support)) |>
    head(10)
```

This example demonstrates how data preparation choices (using raised-cosine vs 
triangular fuzzy sets, choosing appropriate breakpoints, using meaningful labels) 
can make the discovered patterns more interpretable and aligned with domain 
knowledge.
