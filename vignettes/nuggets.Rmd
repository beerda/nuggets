...
The example above searches for association rules where any predicates except
those related to transmission type (`am`) can form the antecedent, while
transmission type predicates serve as consequents. The `disjoint` vector
ensures that mutually exclusive predicates (like `mpg=[low]` and `mpg=[high]`)
don't appear together in the same condition.

The result is a tibble where each row represents one rule, showing the
antecedent, consequent, and quality measures such as support, confidence, lift,
and conviction. Rules are sorted by support to show the most frequently
...
The `shorten_condition()` function creates readable labels by abbreviating
predicate names. Visual properties (fill color, edge width, node size) encode
