...
- **Quality control utilities** to improve pattern mining:
  ...
  - `dig_tautologies()` to find data-driven implications (axioms / near-tautologies)
    that can be used to prune search spaces via the `excluded` argument
...
[Conditional Correlations vignette](conditional-correlations.html).
...
[Contrast Patterns vignette](contrast-patterns.html).
...
grid_callback <- function(d, degrees) {
    if (nrow(d) < 5) return(NULL)  # Skip if too few observations
    
    # Compute weighted correlation
    wcor <- cov.wt(d, wt = degrees, cor = TRUE)$cor[1, 2]
    
    list(
        correlation = wcor,
        n_obs = sum(degrees > 0.1),
        mean_x = weighted.mean(d[[1]], degrees),
        mean_y = weighted.mean(d[[2]], degrees)
    )
}
...
- Explore the [Association Rules vignette](association-rules.html) for a specialized pattern family
- Explore the [Conditional Correlations vignette](conditional-correlations.html) for subgroup-based correlation analysis
- Explore the [Contrast Patterns vignette](contrast-patterns.html) for baseline, complement, and paired contrasts
- Explore the [Custom Patterns vignette](custom-patterns.html) for defining your own pattern types with `dig()`
