...
- *Fuzzy logic*: each predicate is assigned a *truth degree* from the interval
  ...
  *triangular norm (t-norm)*. The package supports three common t-norms:
  
  - *Gödel* (minimum): $\min(a, b)$
  - *Goguen* (product): $a \cdot b$
  - *Łukasiewicz*: $\max(0, a + b - 1)$  
  
  where $a, b \in [0, 1]$ are truth degrees of two predicates.
...
transformations. See the section [Data Preparation](#data-preparation) for more
details.
...
- `dig_paired_baseline_contrasts()` for contrast patterns on paired numeric
  variables,
- `dig_correlations()` for conditional correlations. 

See [Pre-defined Patterns](#pre-defined-patterns) for further details.
