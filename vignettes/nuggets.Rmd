...
- **Quality control utilities** to improve pattern mining:
  ...
  - `dig_tautologies()` to find always-true or almost-always-true rules that
    serve as axioms for pruning search spaces via the `excluded` argument
...
When the dataset contains predicates that are near-always implied by others, the
search may generate many rules whose antecedent or consequent is trivially
deducible. These can be pruned automatically using the `excluded` argument.

The `excluded` argument accepts a list of character vectors, where each vector
encodes one known implication (axiom): all but the last element form the
antecedent and the last element is the consequent. The axioms are used to prune
generated rules via the modus ponens inference rule:

- A rule is pruned if its antecedent contains a predicate that can be deduced
  from the remaining antecedent predicates using the axioms, possibly via a
  chain of multiple axiom applications (transitive deduction). Such a predicate
  is redundant.
- A rule's consequent is pruned (the whole rule is excluded) if it can be
  deduced from the rule's antecedent using the axioms, possibly via a chain of
  multiple axiom applications (transitive deduction).

The list of axioms can be discovered automatically using `dig_tautologies()`.
Use `parse_condition(antecedent, consequent)` to convert the result into the
format expected by `excluded`:

```{r, eval=FALSE}
# Discover near-tautological rules as axioms
tautologies <- dig_tautologies(fuzzy_mtcars,
...
This approach eliminates trivially deducible rules and helps focus on genuinely
