...
public:
    ...
    void deduceConsequentsFromEmptyAntecedent(vector<size_t>& result) const
    { return root.storeConsequentsTo(result); }
     ...
     */
    void deduceConsequentsByRevSorted(const vector<size_t>& prefix,
                                      const size_t predicate,
                                      vector<size_t>& result) const
    {
        ...
        if (node != nullptr) {
            get(node, prefix.rbegin(), prefix.rend(), result);
        }
